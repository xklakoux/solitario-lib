...
public class BaseGame extends Application {
	...
	final public static int NUMBER_OF_SETS = 4;
	final public static int FULL_NUMBER_SET = 13;
	final public static int START_CARDS_DEAL_COUNT = 52;
	...
	public void onCreate() {
		...
		bus = new Bus();
		drawSoundId = sp.load(BaseGame.getAppContext(), R.raw.draw, 1);

		Gson gson = new Gson();
		Type collectionType = new TypeToken<Stack<Move>>() {
		}.getType();

		String previousMoves = BaseGame.getSettings().getString("moves", null);
	...
	public static void setUndid() {
		setsCompleted--;
		getBus().post(BaseGame.GAME_STILL_NOT_WON);
	}
	...
	private static void addToSharedPreferences(List<Card> cards) {
		...
		Editor editor = BaseGame.getSettings().edit();
